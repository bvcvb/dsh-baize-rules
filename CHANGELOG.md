...
- **Config is stricter and wider.** `scope` now also accepts `project` (resolved against the session cwd), and both `scope` and `maxBytes` are **schema-required**: omitting either fails the plugin load with an explicit error instead of silently using an undefined default. A new `apiOriginCheck` option (default `true`) can be turned off when the web UI is reached through a reverse proxy or from another host.
...
- **HTTP API:** `GET`/`POST` responses now carry `problems`; two structured ops were added, `rule.add { scope, text }` and `rule.setEnabled { scope, ruleId, enabled }`; and a refused request now answers with an explicit status instead of an ambiguous body — `400` (refused request, with `text`), `403` (origin refused: this machine only, and a stated browser `Origin` must be same-origin or a local page), `409` (concurrency conflict, reload and retry), `413` (body over 1 MiB) and `500` (`{ error }`).
