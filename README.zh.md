...
| `apiOriginCheck` | `true` | 面板 API 只接受本机请求，且浏览器 `Origin` 必须是宿主自身。当 Web UI 经由反向代理或从其他主机访问时，设为 `false`——此时由代理负责鉴权 |
...
面板把自己注册进两个 dsh 客户端插槽（`ctx.slots.inject('sidebar.footer.action')` 与
`ctx.slots.inject('conversation.view')`），因此有三个入口，数据完全一致：

- **侧边栏底部「规则」按钮**——始终可用，点击即打开（或聚焦）面板。
- **会话内「规则」页签**——会话打开时，面板就是该会话视图环里的一个页签。
- **新会话页**——尚无会话时没有页签环，按钮改为打开一个全屏浮层。
...
- **失败语义**（拒绝时响应体都带 `text`，只有 `500` 返回 `{ error }`）：
  ...
  | `403` | 来源被拒：只接受**本机**请求，且浏览器若声明 `Origin`，必须与宿主同源或来自本地页面（反向代理已自行鉴权时可用 `apiOriginCheck: false` 放宽） |
  | `405` | 非 `GET`/`POST` 方法 |
