...
import { agentCwd, isStaleWrite, readTemplatesWithVersion, view, writeGlobal, writeProject, writeSession, writeTemplates, } from "./store.js";
/** Shown when a write lost a concurrency race; nothing was persisted. */
const STALE_MESSAGE = 'Rules changed elsewhere while this command ran — nothing was written. Run it again.';
...
async function persist(ctx, runtime, sessionId, cwd, before, out, templatesVersion) {
    ...
    if (out.nextTemplates !== undefined)
        await writeTemplates(ctx, out.nextTemplates, templatesVersion);
}
...
export async function handle(ctx, invocation, runtime) {
    ...
    try {
        ...
        if (isStaleWrite(e))
            return { kind: 'error', text: STALE_MESSAGE };
        throw e;
    }
