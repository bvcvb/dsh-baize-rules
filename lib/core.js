...
const USAGE = 'Usage: /baize-rules [<scope>] <command> — scope is one of global|session|project, written '
...
function scopeArgument(scope, args) {
    if (scope !== undefined)
        return scope;
    const first = args[0];
    return first !== undefined && SCOPE_SET.has(first) ? first : undefined;
}
...
export function formatList(view, only) {
    const line = (rule) => {
        const tags = rule.tags ?? [];
        const badge = tags.length > 0 ? `{${tags.join(',')}} ` : '';
        return `[${rule.id.slice(0, 8)}] ${badge}${rule.enabled ? '' : '(disabled) '}${rule.text}`;
    };
    const rulesOf = (scope) => scope === 'project' ? (view.project ?? []) : scope === 'session' ? view.session : view.global;
    const parts = [];
    for (const { scope, header } of LIST_SECTIONS) {
        if (only !== undefined && only !== scope)
            continue;
    ...
    if (parts.length === 0)
        return only === undefined ? 'No active rules.' : `No ${only} rules.`;
    return parts.join('\n');
}
