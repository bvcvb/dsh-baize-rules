...
async function persist(
  ...
  if (nextView !== undefined) {
    ...
    if (sessionId) await writeSession(ctx, sessionId, nextView.session)
    if (project) await writeProject(ctx, project, nextView.project ?? [])
  }
  if (nextTemplates !== undefined) await writeTemplates(ctx, nextTemplates)
}
...
async function applyOp(
  ...
  switch (op) {
    ...
    case 'rule.setTags': {
      ...
      if (findRule(v, scope, id) === undefined) return fail(`Rule ${id} not found in ${scope}.`)
    ...
    case 'rule.saveAsTemplate': {
      ...
      if (out.changed) await persist(ctx, options, sessionId, project, v, undefined, out.templates)
