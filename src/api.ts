...
import type { RuleScope, RuleTemplate, RuleView } from './rules.ts'
...
const STALE_MESSAGE =
  'Rules changed in another window while you were editing — nothing was written. Reload and try again.'
...
function originAllowed(origin: string, host: unknown): boolean {
  if (origin === 'null') return true // file:// pages carry a null origin
  try {
    const parsed = new URL(origin)
    if (typeof host === 'string' && parsed.host === host) return true
 ...
 *  not be enough. While the host binds loopback this is belt-and-braces; it is
 *  what protects the plugin the moment someone binds `0.0.0.0`.
 *
...
function scopeOf(value: unknown): RuleScope {
  return value === 'global' || value === 'project' || value === 'session' ? value : 'session'
}

/** One op's decision, before it is persisted and echoed back to the panel. */
interface OpOutcome {
  readonly ok: boolean
...
function problemsOf(v: RuleView, templates: FileRead<RuleTemplate>): readonly string[] {
  return [
    ...(v.problems ?? []),
    ...
    handler: async (req: IncomingMessage, res: ServerResponse) => {
      ...
      if (method === 'POST') {
        ...
        try {
          ...
          if (e instanceof UnstorableScopeError) sendJson(res, 400, { ok: false, text: e.message })
          // A lost race is retryable, and the client is told exactly that.
          else if (isStaleWrite(e)) sendJson(res, 409, { ok: false, text: STALE_MESSAGE })
...
async function persist(
  ctx: Context,
  options: RulesApiOptions,
  sessionId: string,
  project: string,
  before: RuleView,
  nextView: RuleView | undefined,
  templatesVersion: unknown,
  nextTemplates: readonly RuleTemplate[] | undefined,
): Promise<void> {
  if (nextView !== undefined) {
    if (nextView.session !== before.session && sessionId.length === 0) {
      throw new UnstorableScopeError(
        'No session to store session rules in — this panel is not attached to a conversation. Use the global scope instead.',
      )
    }
    if (nextView.project !== before.project && project.length === 0) {
      throw new UnstorableScopeError(
        'No project directory to store project rules in — this panel has no working directory. Use the global scope instead.',
      )
    }
  ...
  if (nextTemplates !== undefined) await writeTemplates(ctx, nextTemplates, templatesVersion)
}
...
async function applyOp(
  ...
  v: RuleView,
  templatesRead: FileRead<RuleTemplate>,
  options: RulesApiOptions,
  sessionId: string,
  project: string,
): Promise<OpOutcome> {
  const templates = templatesRead.items
  const fail = (text: string): OpOutcome => ({ ok: false, text, view: v, templates })
  const scope = scopeOf(body.scope)
  const persistTemplates = (next: readonly RuleTemplate[] | undefined): Promise<void> =>
    persist(ctx, options, sessionId, project, v, undefined, templatesRead.version, next)

  switch (op) {
    case 'raw': {
      const raw = String(body.raw ?? '')
      if (!raw.trim()) return fail('empty raw')
    ...
    case 'rule.add': {
      ...
      if (text.length === 0) return fail('text required')
    ...
    case 'rule.setTags': {
      ...
      if (!id) return fail('ruleId required')
      const rule = findRule(v, scope, id)
      if (rule === undefined) return fail(`Rule ${id} not found in ${scope}.`)
      const tags = normalizeTags(body.tags as readonly unknown[] | undefined)
    ...
    case 'rule.update': {
      ...
      if (rule === undefined) return fail(`Rule ${id} not found in ${scope}.`)
      const text = String(body.text ?? '').trim()
      const tags = body.tags === undefined ? rule.tags : normalizeTags(body.tags as readonly unknown[])
    ...
    case 'rule.saveAsTemplate': {
      ...
      const rule = findRule(v, scope, id)
      if (rule === undefined) return fail(`Rule ${id} not found in ${scope}.`)
      const out = saveRuleAsTemplate(templates, rule, normalizeTags(body.tags as readonly unknown[] | undefined))
      if (!out.ok) return fail(out.text)
      if (out.changed) await persistTemplates(out.templates)
    ...
    case 'rule.addFromTemplates': {
      ...
      if (refs.length === 0) return fail('templateIds required')
    ...
    case 'template.create': {
      const out = createTemplate(
        templates,
        String(body.text ?? ''),
        normalizeTags(body.tags as readonly unknown[] | undefined),
      )
      if (!out.ok) return fail(out.text)
    ...
    case 'template.update': {
      ...
      const nextTags = body.tags === undefined
        ? undefined
        : normalizeTags(body.tags as readonly unknown[])
    ...
    case 'template.import': {
      ...
      if (payload.length === 0) return fail('payload required')
      const mode: TemplateImportMode = body.mode === 'replace' ? 'replace' : 'merge'
...
async function viewFor(
  ...
  return view(ctx, { id: sessionId } as never, options, project)
}
