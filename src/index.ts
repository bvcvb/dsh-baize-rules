...
/** What the pre-step remembers for one session: the digest of the snapshot it
 *  last published, and the step that carried it. */
...
const lastInjected = /* @__PURE__ */ new WeakMap<Agent['session'], InjectionRecord>()
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.on('agent/pre-step', async (
    ...
    if (!config.injectAtEveryStep && unchanged && !stale) return decision
  ...
  ctx.effect(() => registerRulesApi(ctx, {
    globalRulesPath: config.globalRulesPath,
    originCheck: config.apiOriginCheck === true,
  }), 'baize-rules api')
}
